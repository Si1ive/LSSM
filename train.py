...
def train(model, device, data_loader,num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        print('-' * 60)
        optimizer.zero_grad()# 在 PyTorch中，梯度是累积的，这意味着每次调用 .backward() 方法时，梯度都会累加到现有的梯度上。如果不将梯度清零，那么每次反向传播时，梯度都会累加，导致梯度计算错误。
#---------------------------------------------------------------------------------------------train
        model.train()
        ...
        for data in process_bar:
            ...
            if name_loss == 'Focal_Loss':
                ...
                criterion = Focal_loss.FocalLoss(gamma=2.0, alpha=0.25)
                if name == 'v1':
                    out = model(A,B) #输出的是两通道概率图，criterion中潜入了softmax
                    loss = criterion(out, Label)
                else:
                    # pres = model(A, B)
                    # loss1 = criterion(pres[0], Label)
                    # loss2 = criterion(pres[1], Label)
                    # loss3 = criterion(pres[2], Label)
                    # loss4 = criterion(pres[3], Label)
...
#---------------------------------------------------------------------------------------------val
        ...
        if f_score > best_F1:
            best_F1 = f_score
            best_checkpoint = '/home/zzh/ZZHNet/result/best_checkpoint_' +name+ '/best_statedict_epoch{}_f_score{:.4f}.pth'.format(epoch, f_score)
...
if __name__ == '__main__':
    ...
    plt.savefig('/home/zzh/ZZHNet/result/train_' +name+ '/ZZHNet_Mamba_val_f_score.png')
    plt.figure()
    plt.plot(x, train_loss, 'g', label='train_loss')
    plt.savefig('/home/zzh/ZZHNet/result/train_' +name+ '/ZZHNet_Mamba_train_loss.png')
