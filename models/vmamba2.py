...
class VSSBlock2(nn.Module):
    def __init__(
        ...
        super().__init__()
        #1.共享块
        #因为SS2D不好改，所以没必要专门挪出来了，反正这块刚好都是共享部分
        #因为传入CSSB之前要把AB逐通道相加
        self.ABconv = nn.Conv2d(hidden_dim*2, hidden_dim, kernel_size=1)
        self.norm = norm_layer(hidden_dim)
        #2.CSS2D核心
        self.SS2D = SS2D(
            d_model=hidden_dim,
        ...
        #缩放因子
        self.Agamma = nn.Parameter(torch.tensor(0.1))
        self.Bgamma = nn.Parameter(torch.tensor(0.1))
        self.drop_path = DropPath(drop_path)

        #3.非共享块
        self.Anorm2 = norm_layer(hidden_dim)
        mlp_hidden_dim = int(hidden_dim * mlp_ratio)
    ...
    def forward(self, input):
        ...
        B_0 = B
        #核心
        AB = self.norm(self.ABconv(torch.cat((A, B), dim=1)))
        AB = self.drop_path(self.SS2D(AB))
        #缩放因子
        A = A_0 + AB*self.Agamma
        B = B_0 + AB*self.Bgamma
        #FNN
        A =A + self.drop_path(self.Amlp(self.Anorm2(A)))
        B =B + self.drop_path(self.Bmlp(self.Bnorm2(B)))
        return [A,B]
...
class DDB(nn.Module):
    def __init__(self,channel,norm_layer):
        ...
        self.cc = nn.Sequential(
            nn.Conv2d(channel*2, channel, kernel_size=3, padding=1, groups=channel),
            norm_layer(channel),
        )
        self.GELU = nn.GELU()
    ...
    def forward(self, input):
        A = input[0]
        B = input[1]
        AsubB = self.sub(torch.abs(A - B))
        AcB = self.cc(torch.cat((A,B), dim=1))
        d = self.GELU(AsubB + AcB)
        return d

#将多个CSSM编码层块组织成编码层，主要通过其中makelayer方法
class VSSM(nn.Module):
    def __init__(
        ...
        num_classes=1000,
        depths=[2, 2, 8, 2] ,#层数
        #dims=[128, 256, 512, 1024], #通道数
        dims=[64, 128, 256, 512],  # 通道数
        #dims=[96, 192, 384, 768],  # 通道数
        #dims=[80, 160, 320, 640],
        # =========================
        ssm_d_state=1,
        ssm_ratio=2.0,
        ...
        **kwargs,
        ...
        # 字典的写法，patchembed_version在前文中定义好了，也就是说在初始化时就确定_make_patch_embed版本
    ...
    def _make_patch_embed(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm,
        ...
        return nn.Sequential(
            nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, bias=True),#卷积改变通道，划分补丁,使卷积核大小和步长大小相同，即划分成对应的补丁
            (nn.Identity() if channel_first else Permute(0, 2, 3, 1)),#改变通道优先
            (norm_layer(embed_dim) if patch_norm else nn.Identity()),#是否归一,传入的是通道，所以对每个像素点在通道上进行归一处理
        )

    #增强嵌入，将一次卷积升级为两次，并且中间添加激活和归一
    @staticmethod
